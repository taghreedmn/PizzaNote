   ...
   @endif
    ...
    <div  class="">
      ...
      <tr>
        ...
        <td><b>{{$listings->id}}</b></td>
          ...
          <tr>
            ...
            <td><b>{{$listings->my_f_add_date}}</b></td>
          </tr>
          <tr>
            <td class="text-center bg-infox" style="width: 35%;"><b>الاسم</b></td>
            <td><b>{{$listings->my_f_name}}</b></td>
          </tr>
          <tr>
            <td class="text-center bg-infox" style="width: 35%;"><b>الجوال</b></td>
            <td><b>{{$listings->my_f_mobile}}</b></td>
          </tr>
          <tr>
            <td class="text-center bg-infox" style="width: 35%;"><b>العنوان</b></td>
            <td class="ow"><b>@php echo nl2br($listings->my_f_address) @endphp</b></td>
          </tr>
          <tr>
            <td class="text-center bg-infox" style="width: 35%;"><b>البريد الالكتروني</b></td>
            <td><b>{{$listings->my_f_email}}</b></td>
          </tr>
          <tr>
            <td class="text-center bg-infox" style="width: 35%;"><b>الحالة الاجتماعية</b></td>
            <td><b>{{get_my_f_social($listings->my_f_social)}}</b></td>
          </tr>
        <tr>
          ...
          <div class="accordion-item">
            ...
            <div id="flush-collapseOne" class="accordion-collapse collapse" aria-labelledby="flush-headingOne" data-bs-parent="#div_acc_log">
              <div class="accordion-body">
                @php echo nl2br_sp($listings->myfrind_log); @endphp
              </div>
            </div>
          </div>
        </div>
      ...
      @if (chk_para(session("user_info")["ui_para"], $menu_active."_edit")=="ok" or session("user_info")["ui_type"]=="1")
            <a class="btn btn-info" data-bs-toggle="modal" data-remote="{{ route('myfrind.edit', enc($listings->id) ) }}"  data-bs-target="#modal-edit"><i class="fas fa-edit"></i> تعديل</a>
      @endif
